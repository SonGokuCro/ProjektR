...
class App(Frame):
    ...
    def generate_keys(self):
        self.private_key = falcon.SecretKey(n = 128)
        self.public_key = falcon.PublicKey(self.private_key)
        self.public_key_value['text'] = ("".join(hex(i)[2:] for i in self.public_key.h))[:self.SHOWN_CHARS] + "..."
        self.private_key_value['text'] = ("".join(hex(i)[2:] for i in self.private_key.f))[:self.SHOWN_CHARS] + "..."
    ...
    def generate_signature(self):
        ...
        self.message = self.message_box.get(1.0, END).strip('\n')
        string = bytes(self.message, "ascii")
        self.signature = self.private_key.sign(string)
        self.signature_value['text'] = self.signature.__repr__()[:self.SHOWN_CHARS_SIG] + "..."
    ...
    def verify_signature(self):
        ...
        if(self.private_key.verify(string, self.signature)):
    ...
    def save_public_key_edit(self):
        ...
        if(len(bytes) != falcon.PUBLIC_BYTES or not self.is_hex(bytes)):          
    ...
    def save_private_key_edit(self):
        ...
        if(len(bytes) != falcon.PRIVATE_BYTES or not self.is_hex(bytes)):          
            mbox.showwarning(title="Upozorenje", message="Netočan unos!")
        else:
            mbox.showinfo(title="Obavijest", message="Promjene spremljene")
    ...
    def save_signature_edit(self):
        my_bytes = self.textbox_signature.get(1.0, END).strip().split(' '); print(my_bytes);
        if(len(my_bytes) != len(self.signature) or not self.is_hex(my_bytes)):  
            mbox.showwarning(title="Upozorenje", message="Netočan unos!")
        else:
            #mbox.showinfo(title="Obavijest", message="Promjene spremljene")
            self.signature = bytes(int(i, 16) for i in my_bytes)
    ...
    def textbox_public_key_window(self):
        ...
        self.textbox_public_key.insert("1.0", repr(self.public_key.h))
    ...
    def textbox_private_key_window(self):
        ...
        self.textbox_private_key.insert("1.0", repr(self.private_key))
    ...
    def textbox_signature_window(self):
        ...
        save_button = Button(button_frame, text="Spremi i izađi", relief="solid", fg="black",highlightbackground = "black", highlightthickness=1, bd=0, font=self.button_font, bg="#f5f5f5", command=(lambda: (self.save_signature_edit(), popup.destroy())))
        save_button.pack(side=LEFT, padx=20)
    ...
    def home_screen(self):
        ...
        title_label = Label(self, text="FALCON", font=title_font, bg="white")
